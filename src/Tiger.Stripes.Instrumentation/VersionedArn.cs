// <copyright file="VersionedArn.cs" company="Cimpress plc">
...
readonly ref struct VersionedArn(ReadOnlySpan<char> partition, ReadOnlySpan<char> region, ReadOnlySpan<char> accountId, ReadOnlySpan<char> resource, ReadOnlySpan<char> version)
{
    ...
    readonly ReadOnlySpan<char> _partition = partition;
    readonly ReadOnlySpan<char> _region = region;
    readonly ReadOnlySpan<char> _accountId = accountId;
    readonly ReadOnlySpan<char> _resource = resource;
    readonly ReadOnlySpan<char> _version = version;

    /// <summary>Gets the Account ID from this ARN.</summary>
    public string AccountId { get; } = new(accountId);
    ...
    {
        var totalLength = Arn.Length + _partition.Length + Lambda.Length + _region.Length + _accountId.Length + Function.Length + _resource.Length + _version.Length + 7;
        return string.Create(totalLength, this, static (span, state) =>
        {
            ...
            Apply(state._accountId, span, ref index);
